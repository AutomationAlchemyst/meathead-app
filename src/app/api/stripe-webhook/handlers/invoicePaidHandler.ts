...
export async function handleInvoicePaid(invoice: Stripe.Invoice) {
  ...
  if (!firebaseUserId) {
    throw new Error(`Could not determine firebaseUserId for paid invoice ${invoice.id}`);
  }

  console.log(`[Webhook Handler] Processing successful invoice payment for user ${firebaseUserId}`);
  ...
  console.log(`[Webhook Handler] User ${firebaseUserId} subscription confirmed active on invoice payment.`);
}

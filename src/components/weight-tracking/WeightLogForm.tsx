...
// FIX: Add an optional onLogSuccess callback prop
interface WeightLogFormProps {
  onLogSuccess?: () => void;
}

export default function WeightLogForm({ onLogSuccess }: WeightLogFormProps) {
  ...
  const onSubmit = async (data: WeightLogFormValues) => {
    ...
    try {
      ...
      // FIX: If the callback exists (i.e., the form is in a modal), call it.
      if (onLogSuccess) {
        onLogSuccess();
      } else {
        // If it's on its own page, refresh the page as before.
        router.refresh(); 
      }
